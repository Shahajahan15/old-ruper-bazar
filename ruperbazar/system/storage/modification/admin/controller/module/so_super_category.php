...
class ControllerModuleSosupercategory extends Controller {
	...
	// Delete Module
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('module/so_super_category', 'token=' . $this->session->data['token'], 'SSL'));
		}
	//Get Module Id New
		$moduleid_new= $this->model_module_so_super_category->getModuleId(); 
		$module_id = '';	
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (!isset($this->request->get['module_id'])) {
				...
				$this->model_extension_module->addModule('so_super_category', $this->request->post);
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('module/so_super_category', 'module_id='.$module_id.'&token=' . $this->session->data['token'], 'SSL'));
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('module/so_super_category', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/so_super_category', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		$categorys_list = $this->model_module_so_super_category->getCategories($filter_data);
	...
	// ---------------------------Load module --------------------------------------------
		...
		$data['moduletabs'] = $this->model_extension_module->getModulesByCode( 'so_super_category' );
		$data['link'] = $this->url->link('module/so_super_category', 'token=' . $this->session->data['token'] . '', 'SSL');
	// ---------------------------Load data ----------------------------------------------
	...
	// Effect 
		$data['effects'] = array(
			...
			'hatch' 		=> $this->language->get('effect_hatch'),
			'bounce' 		=> $this->language->get('effect_bounce'),
	...
	//Get Data Default
		...
		$this->response->setOutput($this->load->view('module/so_super_category.tpl', $data));
	}
	...
	public function _breadcrumbs(){
		...
		if (!isset($this->request->get['module_id'])) {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('module/so_super_category', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('module/so_super_category', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
		return $this->data['breadcrumbs'];
	}
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/so_super_category')) {
	...
	// validate subcategory slideby		
		if (!filter_var($this->request->post['subcategory_slideby'],FILTER_VALIDATE_FLOAT) || $this->request->post['subcategory_slideby'] < 0) {
			$this->error['subcategory_slideby'] = $this->language->get('error_subcategory_slideby');
		}
