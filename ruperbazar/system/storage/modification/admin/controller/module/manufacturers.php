...
class ControllerModuleManufacturers extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/manufacturers', 'token=' . $this->session->data['token'], 'SSL')
		);

		$data['action'] = $this->url->link('module/manufacturers', 'token=' . $this->session->data['token'], 'SSL');

		$data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');

		if (isset($this->request->post['category_status'])) {
			$data['manufacturers_status'] = $this->request->post['manufacturers_status'];
		} else {
			$data['manufacturers_status'] = $this->config->get('manufacturers_status');
		}
