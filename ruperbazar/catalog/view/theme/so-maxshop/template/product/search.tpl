...
/*SET CATEGORY COLUMNS*/	
	...
	/*$change_limits = reset($limits);
	$array_str=array();
	foreach ($change_limits as  $key=>$value) {
		if ($value) $value = preg_replace('/6/', '8', $value); 
		$array_str[$key] = $value;
	}
	array_shift($limits);
	array_unshift($limits,$array_str);
	*/
?>
				...
				<div class="left-block">
					...
					<?php if (!isset($quick_status) || $quick_status != 0) : ?>
						<!--full quick view block-->
							...
							<a class="quickview iframe-link visible-lg" data-toggle="tooltip" title="Quickview" data-fancybox-type="iframe"  href="<?php echo $our_url->link('product/quickview','product_id='.$product['product_id']);?>">  <?php echo (isset($quick_view_text) ? $quick_view_text : 'QuickView'); ?></a>
